...
- Letter bar focus lost when touching a touchscreen remote overlay: a `ViewTreeObserver.OnGlobalFocusChangeListener` now detects when focus is cleared without a navigation target (touch-mode entry) and immediately restores it via `requestFocusFromTouch()`. Views removed from the tree (e.g. after ENTER) are excluded via `isAttachedToWindow()`.
